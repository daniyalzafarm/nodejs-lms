...
router.put("/assign/:cid/Student/:sid", function (req, res, next) {
  Class.findOneAndUpdate(
    ...
    { new: true, upsert: false }, // upsert method specify that wheather the oject is created if not found
...
router.put("/class/:cid", function (req, res, next) {
  ...
  const allowedUpdates = ["name", "teacher", "students"];
  ...
  Class.findOneAndUpdate({ _id: req.params.cid }, req.body, {
    new: true,
    upsert: false,
  }).then((result) => {
    if (!result) {
